...
Stri.prototype = $extend(Term.prototype,{
	...
	,stringify: function(sb) {
		if(Parser.escapeIsNeeded(this.vp)) {
			sb.b += "\"";
			Parser.escapeSymbol(sb,this.vp);
			sb.b += "\"";
		} else sb.addSub(this.vp,0,null);
	}
	,prettyStringify: function(sb) {
		this.stringify(sb);
	}
	...
	,buildJsonString: function(sb) {
		sb.b += "\"";
		if(Parser.escapeIsNeeded(this.vp)) Parser.escapeSymbol(sb,this.vp); else sb.addSub(this.vp,0,null);
		sb.b += "\"";
	}
	,__class__: Stri
});
...
Seqs.prototype = $extend(Term.prototype,{
	...
	,baseLineStringify: function(sbuf) {
		if(this.sp.length >= 1) {
			this.sp[0].stringify(sbuf);
			var _g1 = 1;
			var _g = this.sp.length;
			while(_g1 < _g) {
				var i = _g1++;
				sbuf.b += " ";
				this.sp[i].stringify(sbuf);
			}
		}
	}
	,baseLinePrettyStringify: function(sbuf) {
		if(this.sp.length >= 1) {
			this.sp[0].prettyStringify(sbuf);
			var _g1 = 1;
			var _g = this.sp.length;
			while(_g1 < _g) {
				var i = _g1++;
				sbuf.b += " ";
				this.sp[i].prettyStringify(sbuf);
			}
		}
	}
	,prettyStringify: function(sb) {
		if(this.sp.length == 0) sb.b += ":"; else if(this.sp.length == 2) {
			this.sp[0].prettyStringify(sb);
			sb.b += ":";
			this.sp[1].prettyStringify(sb);
		} else {
			var starts;
			if(this.sp[0].v() != null) {
				this.sp[0].stringify(sb);
				sb.b += "(";
				if(this.sp.length >= 1) this.sp[1].prettyStringify(sb);
				starts = 2;
			} else {
				sb.b += "(";
				this.sp[0].prettyStringify(sb);
	...
	,buildJsonString: function(sbuf) {
		sbuf.b += "[";
		if(this.sp.length > 0) this.sp[0].buildJsonString(sbuf);
		var _g1 = 1;
		var _g = this.sp.length;
		while(_g1 < _g) {
			var i = _g1++;
			sbuf.b += ",";
			this.sp[i].buildJsonString(sbuf);
		}
		sbuf.b += "]";
	}
	,__class__: Seqs
